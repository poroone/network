...
+ if-Modified-Since
    - 如果上一次的响应头中没有ETag,有Last-Modeified,就会将L;ast-Modified的值作为请求头的值
    - 如果服务器发现资源的最后一次修改时间晚于If-Modified-Since,就会返回新的资源(200 OK)

## 缓存 - Last-Modified 和 ETag
+ Last-Modified的缺陷
    - 只能精确到秒级别，如果资源在一秒内被修改了,客户端将无法获取最新的资源数据
    - 如果某些资源被修改了,(最后一次修改时间发生变化了),但是内容并没有任何变化
        * 会导致相同数据重复传输,没有使用到缓存
+ ETag可以办到
    - 只要资源的内容没有发生变化,就不会重复传输资源数据
...
+ IPV6(Internet Protocol version 6),译为:网际协议第6版
    - 用它来取代IPV4主要是为了解决IPV4地址枯竭问题,同时他也在其他方面对于IPV4有许多改进
    - 然而长期以来IPV4在互联网流量中仍占据主要地位,IPV6的使用增长缓慢
    - 在2019年12月,通过IPV6使用Google服务的用户百分率首次超过30%
        * 应为需要设备,操作系统内核升级支持ipv6
+ IPV6采用128位的地址,而IPV4使用的是32位
    - 地球70亿人算,平均每个人可以分得约4.86*10^28个ipv6地址

## IPV6 - 地址格式
+ ipv6地址为128bit,每16bit一组,共8组
+ 每组以冒号 ":" 隔开,魅族一4位十六进制方式表示 0-9 a-f
    - 例如2002:25df:563a:bd56:2002:25df:563a:bd56
+ 类似于IPV4的点分十进制,同样也存在点分十六禁止的写法
    - 2.0.0.2.2.5.d.f.5.6.3.a.b.d.5.6.2.0.0.2.0.2.5.d.f.5.6.3.a.b.d.5.6
+ 每组前面连续的0可以省略.下面的ipv6地址是等价的
    - 2021:0adf:025d:0000:0000:0000:0000:0e13
    - 2021:adf:25d:0:0:0:0:e13
+ 可以用双冒号 "::" 表示一组0或多组连续的0,但只能出现一次,下面的IPV6是等价的
    - 2021:adf:25d:0:0:0:0:e13
    - 2021:adf:25d::e13
+ 2021::adf:25d::e13是非法的,应为双冒号出现了两次,会造成歧义
    - 2021:0adf:025d:0000:0000:0000:0000:0e13
    - 2021:0000:0000:0000:0000:0adf:025d:0e13
...
+ Traffic Class(占8bit):交通类别
    - 只是数据包的类别或优先级,可以帮助路由器根据数据包的优先级处理流量
    - 如果路由器发生拥塞,则优先级最低的数据包将被丢弃
+ Paylod Length(占16bit):有效负载长度
    - 最大值65535字节
    - 包括了扩展头部,上层(传输)数据的长度
...
## ![](https://cdn.nlark.com/yuque/0/2024/png/26748482/1732542781983-768fc2d0-11fc-4651-9f17-cef47fa816c3.png)
![](https://cdn.nlark.com/yuque/0/2024/png/26748482/1732542850058-568733e4-6e22-4a07-a141-1347b66a8a75.png)

## IPV6 - 扩展头部
+ Next Header(占8bit):下一个头部
    - 指示扩展头部(如果存在)的类型,上层数据包的协议类型(例如TCP，UDP，ICMPV6)
    - ![](https://cdn.nlark.com/yuque/0/2024/png/26748482/1732544349956-7c78f0b3-e524-46ed-a2de-f7c5cdc18552.png)

![](https://cdn.nlark.com/yuque/0/2024/png/26748482/1732543599167-8ea128e9-99bc-4d1d-b7bd-641dd603e409.png)

![](https://cdn.nlark.com/yuque/0/2024/png/26748482/1732543832898-12bf34a8-67ea-4fc3-9680-43ee7298db0e.png)

# 即时通信
+ 即时通信(Instant Message 简称IM)平时用的QQ，微信都属于IM应用
+ 国内的IM开发者社区
    - http://www52im.net
+ Im云服务
    - 网易云信,腾讯云,环信等
...
## 即时通信 - MQTT
+ MQTT(Message Queuing Telemetry Transport)译为消息队列遥测传输
    - 基于TCP,默认端口1883 8883(带SSL/TLS)
+ 特点
    - 开销很小,以降低网络流量，信息陈宇远小于XMPP
    - 不是专门为IM设计的协议,很多功能需要自己实现
    - 很多人认为MQTT是最适合物联网(loT Inter of Things)的网络协议
    -   ![](https://cdn.nlark.com/yuque/0/2024/png/26748482/1732548692227-3b04e13e-9f6f-4b94-ae06-20b2ffd95fc8.png)
    - 发布者客户端
    - 代理:服务器
    - 订阅者客户端
...
+ 流媒体(Streaming Media),又叫流式媒体
    - 是指将一连串的多媒体数据压缩后，经过互联网分段发送数据,在互联网上即时传输影音以供欣赏的一种技术
    - 此技术是的资料数据包得以像流水一样发送,不适用此技术,就必须在使用前下载整个媒体文件

## 流媒体 - 常见协议
+ RTP(Real -Time Transport Protocol)译为:实时传输协议
    - 参考RFC 3550 RFC3551基于UDP
+ RTCP(Real -Time Transport Control Protocol) 译为:实时传输控制协议
    - 参考RFC 3350 基于UDP,使用RTP的下一个端口
+ RTSP(Real Time Streaming Protocol)译为:实时流协议,参考:RFC 7820
    - 基于TCP UDP的554端口
+ RTMP (Real-Time Message Protocol),译为:实时消息传输u协议,有Adobe公司出品
    - 基于TCP的1935端口
+ HLS(HTTP Live Streaming),基于HTTP的流媒体网络传输协议,苹果公司出品.
    - 参考RFC 8216

# 当一个访问一个api的时候会发生什么
客户端 ->交换机->路由器->路由器->交换机->服务端

1. 浏览器输入http:baidu.com
2. 询问DNS获取百度服务器的IP地址
3. 发送HTTP请求(调用Socket API发送请求)
4. 建立连接(TCP三次握手)
5. 发送HTTP请求报文
6. 返回HTTP响应报文
